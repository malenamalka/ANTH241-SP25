...
b. What is the p-value in the ANOVA test?
...
10. Let's practice using the Kruskal-Wallis and post-hoc Dunn Test as the non-parametric ANOVA. [We learned this in exercise 7]. Let's see if there are differences moderate exercise activity by mental health status based on the "FeelBad" variable.
```{r}
#1. Generate descriptive stastics of Moderate by Feelbad
#data_set%>%
  #drop_na(DepVar)%>%
  #group_by(IndVar)%>%
  #summarize(mean(DepVar), sd(DepVar), count=n())

#2. Run the Kruskal-Wallis and Dunn tests:
#dunn.test(x=data_set$DepVar, g=data_set$IndVar, kw=TRUE)
